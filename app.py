...
async def process_endpoint(
    ...
    groq_key = x_groq_key or request.headers.get("X-Groq-Key")
    if not gemini_key and not groq_key:
        raise HTTPException(status_code=400, detail="Missing X-Gemini-Key or X-Groq-Key header")
...
async def thumbnail_analyze(
    ...
    api_key = x_gemini_key or x_groq_key
    if not api_key:
        raise HTTPException(status_code=400, detail="Missing X-Gemini-Key or X-Groq-Key header")
...
async def saasshorts_analyze(
    ...
    if not gemini_key and not groq_key:
        raise HTTPException(status_code=400, detail="Missing Gemini or Groq API Key")
    ...
    try:
        ...
        def run_analysis():
            ...
            if req.url and req.url.strip():
                ...
                analysis = analyze_saas(scraped, gemini_key, web_research=web_research, groq_key=groq_key)
            else:
...
async def gallery_html_page():
    ...
    for i, v in enumerate(videos):
        ...
        ld_items.append(f'{{"@type":"ListItem","position":{i+1},"url":"https://moneyfest.app/video/{video_id}","name":"{title}"}}')
...
<title>AI UGC Video Gallery | Moneyfest</title>
...
<meta property="og:title" content="AI UGC Video Gallery | Moneyfest">
...
<body>
<nav><strong style="font-size:18px">Moneyfest</strong><a href="/" class="cta">Create Your Video</a></nav>
...
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>{title} - AI UGC Video | Moneyfest</title>
...
<body>
<nav><strong>Moneyfest</strong><a href="/gallery">Gallery</a><span style="color:#3f3f46">›</span><span style="color:#e4e4e7;font-size:14px">{title}</span></nav>
<div class="container">
