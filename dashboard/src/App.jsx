...
const SECRET_KEY = import.meta.env.VITE_ENCRYPTION_KEY || "Moneyfest-Static-Salt-Change-Me";
...
const SESSION_KEY = 'moneyfest_session';
...
function App() {
  ...
  const handleProcess = async (data) => {
    if ((!apiKey && !groqKey) || !uploadPostKey) {
    ...
    <div className="w-20 lg:w-64 bg-surface border-r border-white/5 flex flex-col h-full shrink-0 transition-all duration-300">
      <div className="p-6 flex items-center gap-3">
        <div className="w-8 h-8 bg-white/5 rounded-lg flex items-center justify-center shrink-0 overflow-hidden border border-white/5">
          <img src="/logo-moneyfest.svg" alt="Moneyfest logo" className="w-full h-full object-cover" />
        </div>
        <span className="font-bold text-lg text-white hidden lg:block tracking-tight">Moneyfest</span>
      </div>
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden selection:bg-primary/30">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        {((!apiKey && !groqKey) || !uploadPostKey) && activeTab !== 'settings' && (
            ...
            <div className="flex items-center gap-3 text-sm text-amber-200">
              ...
              <div>
                ...
                <span className="text-amber-200/80">
                  {!apiKey && !uploadPostKey
                    ? 'Set your Gemini or Groq and Upload-Post API keys to use Moneyfest.'
                    : !apiKey
                      ? 'Set your Gemini or Groq API key to use Moneyfest.'
                      : 'Set your Upload-Post API key to use Moneyfest.'}
                </span>
              </div>
            </div>
            <button
        ...
        <div className="flex-1 overflow-hidden relative">
          ...
          {activeTab === 'settings' && (
            <div className="h-full overflow-y-auto p-8 max-w-2xl mx-auto animate-[fadeIn_0.3s_ease-out]">
              ...
              <KeyInput onKeySet={setApiKey} savedKey={apiKey} label="Gemini API Key" placeholder="AIzaSy..." helpUrl="https://aistudio.google.com/app/apikey" />
              <KeyInput onKeySet={setGroqKey} savedKey={groqKey} label="Groq API Key" placeholder="gsk_..." helpUrl="https://console.groq.com/keys" />
          ...
          {activeTab === 'saasshorts' && (
            <SaaShortsTab geminiApiKey={apiKey} groqApiKey={groqKey} elevenLabsKey={elevenLabsKey} falKey={falKey} uploadPostKey={uploadPostKey} uploadUserId={uploadUserId} />
          )}
          ...
          {activeTab === 'thumbnails' && (
            <ThumbnailStudio geminiApiKey={apiKey} groqApiKey={groqKey} uploadPostKey={uploadPostKey} uploadUserId={uploadUserId} />
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowKeyModal(false)}>
          <div className="bg-[#18181b] border border-white/10 rounded-2xl p-6 max-w-md w-full mx-4 space-y-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-zinc-400">
              Moneyfest needs either a <strong className="text-zinc-200">Gemini</strong> or <strong className="text-zinc-200">Groq</strong> API key, plus an <strong className="text-zinc-200">Upload-Post</strong> API key. Both providers have free tiers.
            </p>
