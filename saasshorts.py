...
from ai_providers import best_available_text_provider, parse_json_text
...
def analyze_saas(scraped_data: dict, gemini_key: str, web_research: dict = None, groq_key: str = None) -> dict:
    """
...
Include 5-8 pain points, 4-6 emotional hooks, and 4+ viral angles."""
    ...
    try:
        text, provider = best_available_text_provider(
            prompt=prompt,
...
- Example male: "a 29 year old european man, short dark hair, light stubble, wearing a navy t-shirt, smart casual look" """
    ...
    try:
        ...
        scripts = json.loads(text[text.find("["): text.rfind("]") + 1] if "[" in text and "]" in text else text)
    except json.JSONDecodeError as e:
        raise Exception(f"Failed to parse scripts JSON: {e}\nRaw: {text[:500]}")

    for script in scripts:
        script["_provider"] = provider
